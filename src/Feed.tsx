...
export default function (): JSX.Element {
  ...
  useEffect(() => {
    if (parentLoginInfo.isLoggedIn) {
      axios
        .post('http://127.0.0.1:41037/api/users/licks', {
  ...
  function handleSubmit(event: { preventDefault: () => void }): void {
    ...
    axios.post('http://127.0.0.1:41037/api/licks', {
  ...
  function retrieveSavedLicks(id: string): void {
    if (!parentLoginInfo.isLoggedIn) {
      ...
      axios.patch('http://127.0.0.1:41037/api/users/licks', {
        email: parentLoginInfo.email,
        uuid: id,
      })
      axios
  ...
  */ 
  ...
  const refreshFeed = (): void => {
    axios.get('http://127.0.0.1:41037/api/licks').then((res) => {
